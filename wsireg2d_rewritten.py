...
class WsiReg2D(object):
    ...
    def _transform_write_image(
        ...
        if (
            ...
            im_fp = ometiffwriter.write_image_by_plane_new(
                output_path.stem,
                output_dir=str(self.output_dir),
            )

            im_dapi = ometiffwriter.write_image_by_plane_DAPI(
                output_path.stem,
                output_dir=str(self.output_dir),
            )

        return im_fp, im_dapi
    ...
    def _transform_write_merge_images(self, to_original_size=True):
        ...
        im_dapi = merge_ometiffwriter.merge_write_image_by_plane_DAPI(
            output_path.stem,
            sub_images,
            output_dir=str(self.output_dir),
        )
