...
class MergeOmeTiffWriter:
    ...
    def _create_channel_names_DAPI(self, sub_image_names):
        """Create channel names for merge data."""
    ...
    def _create_channel_names_new(self, sub_image_names):
        """Create channel names for merge data."""
    ...
    def merge_write_image_by_plane_new(
        self,
        image_name: str,
        ...
        with TiffWriter(output_file_name, bigtiff=True) as tif:
            for m_idx, merge_image in enumerate(self.reg_image.images):
                ...
                #The first image in the image list has all channels - all other images have all except for one 
                if m_idx == 0: img_range = range(merge_n_ch)
                else: img_range = range(1, merge_n_ch)

                for channel_idx in img_range:
                    if self.reg_image.images[m_idx].reader != "sitk":
    ...
    def merge_write_image_by_plane_DAPI(
        self,
        image_name: str,
        sub_image_names: List[str],
        output_dir: Union[Path, str] = "",
        ...
        with TiffWriter(output_file_name, bigtiff=True) as tif:
            for m_idx, merge_image in enumerate(self.reg_image.images):
                ...
                print(
                    f" writing subimage index {m_idx} : {sub_image_names[m_idx]} - "
                    f"channel index - {0} - shape: {image.shape}"
                )
                tif.write(
                    image,
                    subifds=self.subifds,
